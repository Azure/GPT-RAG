...
- **Key Vault-backed audit pseudonymization key.** Post-provisioning creates `AUDIT-HMAC-KEY` from 256 cryptographically random bits only when the secret is absent, reuses it on later runs, and registers only an `AUDIT_HMAC_KEY` Key Vault reference in App Configuration. The value is not placed in plaintext configuration, deployment output, or logs. Rotation is explicit: create a new Key Vault secret version and advance `AUDIT_HMAC_KEY_ID`.
