...
### Validation
Validated in Azure with the Zero-Trust topology (`NETWORK_ISOLATION=true`, env `gptrag-0601261130`, RG `rg-gptrag-0601261130`, `francecentral`): `az bicep build --file .\infra\main.bicep` succeeded with only pre-existing warnings, and `azd provision --no-prompt` completed with the Windows jumpbox Custom Script Extension using the pinned landing zone `v2.0.12`.

Data-plane post-provision/deploy validation was started from the jumpbox (`RUN_FROM_JUMPBOX=true`) and uncovered two Azure/environment blockers outside the GPT-RAG pin change: AI Foundry RAI policy creation repeatedly returned `InternalServerError: Encountered internal server error. Please try again`, and component `azd deploy` could not complete in this existing isolated environment because ACR remote builds require a private ACR task agent pool. The environment had been provisioned with `DEPLOY_ACR_TASK_AGENT_POOL=false`; enabling it afterward failed at the Azure Container Registry agent pool resource with `provisioningState=Failed` for `build-pool`, while public ACR Tasks were denied by the private registry firewall (`client with IP '98.66.144.25' is not allowed access`). The release branch therefore records successful infrastructure validation for the landing-zone bump, with end-to-end service deployment blocked by these external data-plane/build-pool failures.
