...
- **Generic MCP Server knowledge source template (preview, Azure/gpt-rag#567).** `config/search/search.j2` renders the `mcpServer` Foundry IQ knowledge source (Search API `2026-05-01-preview`) when `FOUNDRY_IQ_MCP_ENABLED=true` and at least one source is configured in `FOUNDRY_IQ_MCP_SOURCES_JSON`. Off by default; disabled rendering is byte-for-byte unchanged (`models: []`, `retrievalReasoningEffort: minimal`). When enabled, the knowledge base's `models` array reuses the same Azure OpenAI model object already rendered for standard-mode Blob content extraction as the planning model, and `retrievalReasoningEffort` is set to `low` or `medium` via `FOUNDRY_IQ_MCP_REASONING_EFFORT`. Each tool's `outputParsing` renders as the official Search API object shape (`{"kind": "auto"|"none"|"split"}` or `{"kind": "json", "jsonParameters": {"documentsPath": ...}}`), matching `McpServerOutputParsing`/`McpServerJsonOutputParsing`. Coexists with all existing knowledge sources (Blob, Search index, Work IQ, Fabric ontology/Data Agent, SharePoint remote/indexed, OneLake, Web).
- **`config/search/foundry_iq_mcp_setup.py`: fail-closed validation for MCP sources.** Unlike the soft/best-effort preflight pattern used by some other opt-in sources, `config/search/setup.py` now aborts provisioning with an actionable error when `FOUNDRY_IQ_MCP_ENABLED=true` and the configuration is invalid: malformed JSON, zero sources or tools, duplicate names, non-`https` `serverURL`, userinfo/query string/fragment/IP-literal/`localhost` hosts, a host outside `FOUNDRY_IQ_MCP_TRUSTED_HOSTS`, unsupported `outputParsing`/`inclusionMode` values, a missing (or, for non-`json` kinds, present) `documentsPath`, non-positive or over-cap (`8192`) `maxOutputTokens`, an `alwaysQuerySource` field (never emitted for MCP), or any `auth`/`authentication` key or literal credential-shaped key (API key, bearer token, password, secret, stored header) anywhere in the JSON, at any nesting depth. No MCP authentication metadata is supported or forwarded in this release; every MCP source relies solely on implicit managed-identity authentication. A standalone `python -m config.search.foundry_iq_mcp_setup` CLI entry point runs this same validation from process environment variables as a `scripts/postProvision.ps1` pre-flight gate, before any setting is imported into Azure App Configuration. When disabled, the pre-flight skips source parsing and provisioning writes canonical `[]` plus safe MCP defaults.
...
- **`FOUNDRY_IQ_MCP_*` App Configuration passthrough.** `postProvision.ps1` seeds `FOUNDRY_IQ_MCP_ENABLED`, `FOUNDRY_IQ_MCP_SOURCES_JSON`, `FOUNDRY_IQ_MCP_REASONING_EFFORT`, `FOUNDRY_IQ_MCP_TRUSTED_HOSTS`, and `FOUNDRY_IQ_MCP_LOG_TOOL_ARGUMENTS` with safe defaults so a fresh provision registers the keys without any config action from operators who do not opt in. The Foundry IQ MCP source pre-flight validation above always runs before this seeding step imports anything into App Configuration.
- **Docs: [Foundry IQ: Generic MCP server](https://azure.github.io/GPT-RAG/howto_grounding_mcp_server/) how-to (preview).** Covers the trusted-host and tool allowlist model, the managed-identity-only authentication boundary, API Management / gateway and rate-limiting guidance, the Azure Monitor MCP reference scenario (workspace-scoped Log Analytics Data Reader, bounded time range and row caps, auditable KQL), rollout/canary/rollback via the existing disable gate, and production blockers.
