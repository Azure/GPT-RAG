...
compatible runtime; they do not define how long Foundry Conversations must be
retained or satisfy records-management obligations.

The delegated primary path provisions no capability key or continuity Key
Vault reference. Capability/HMAC remains a disabled fallback only. If a prior
fallback deployment exists, disabled reconciliation can remove its exact
reference and secret-scoped role while retaining secret-version history.
Operators must define when fallback key versions and managed Conversations are
deleted, who can perform those deletions, and what rollback or investigation
period is required.
