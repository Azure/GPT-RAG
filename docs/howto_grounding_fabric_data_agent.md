...
!!! warning "Unmerged compatibility change: no silent local-only answer"
    ...
    providers. An enabled Fabric Data Agent source makes the whole mixed
    request user-required, even with `ALLOW_ANONYMOUS=true`. Missing/failed OBO
    rejects retrieval; no app-identity fallback is enabled. Restore signed-in
    token forwarding, audience, consent and Fabric permissions, or operationally
