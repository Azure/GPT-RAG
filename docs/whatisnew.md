...
- **Fabric Data Agent grounding (preview, [v3.4.2](https://github.com/Azure/GPT-RAG/releases/tag/v3.4.2)).** Answers can now be grounded on a Microsoft Fabric Data Agent (a virtual analyst that queries Fabric data) alongside documents, Work IQ, and the Fabric ontology source. Off by default, same OBO auth and data-egress caveat as the other Fabric source. See [Foundry IQ: Fabric Data Agent](howto_grounding_fabric_data_agent.md).
