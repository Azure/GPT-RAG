...
### June 2026
**[Release 2.9.17](https://github.com/Azure/GPT-RAG/tree/v2.9.17) - Conversations detail dialog renders messages again**

Hotfix on top of v2.9.16 that bumps the orchestrator pin from `v2.8.12` to [`v2.8.13`](https://github.com/Azure/gpt-rag-orchestrator/releases/tag/v2.8.13). No other component changed; UI, ingestion, and AI Landing Zone pins are identical to v2.9.16. After the v2.9.16 fix that reconstructs Conversations detail from `questions[]`, the dialog still showed `user (empty)` / `assistant (empty)` cards for every turn. The backend was emitting each message body under a `text` field (the same name used in the persisted `questions[]` schema), but the dialog component only read `content`, so the message body never made it to the screen. The dialog now reads `content` and falls back to `text`, so reconstructed user prompts and assistant answers render correctly without any backend redeploy gymnastics.

**[Release 2.9.16](https://github.com/Azure/GPT-RAG/tree/v2.9.16) - Overview and Conversations dashboard fixes**

Patch release that bumps the orchestrator pin from `v2.8.11` to [`v2.8.12`](https://github.com/Azure/gpt-rag-orchestrator/releases/tag/v2.8.12). No other component changed; UI, ingestion, and AI Landing Zone pins are identical to v2.9.15. Five operator-reported fixes against the dashboard's **Overview** and **Conversations** tabs. (1) On the Overview tab the **Custom range** From/To date inputs accepted dates past today, so operators could ask for windows that included tomorrow and were silently empty; both inputs now cap at today (UTC), any future date previously saved to `localStorage` is clamped on load, and an inline "Dates can't be in the future." error mirrors the existing 365-day cap message. (2) The `to=YYYY-MM-DD` query parameter on `GET /api/dashboard/overview` was being parsed as midnight UTC of that day and used as an inclusive upper bound, which meant every conversation created later in the day was excluded; the parser now treats a bare `YYYY-MM-DD` as **end of day UTC** when used for `to`, so picking From=2026-06-15 To=2026-06-19 now actually includes everything that happened during 2026-06-19. (3) The Overview tooltips rendered on a semi-transparent background that overlaid the card text behind them, making the tooltip body unreadable; the popover now uses an opaque `bg-card` background with a visible border and a small shadow so it sits cleanly above the cards in both light and dark mode (the v2.8.11 sentence-case fix is preserved). (4) Clicking any row in the **Conversations** tab opened a dialog with empty `user (empty)` / `assistant (empty)` cards, because assistant replies live on the Azure AI Foundry agent thread (referenced by `thread_id`) and only the user prompts are persisted in Cosmos (`questions[]`); the dashboard detail endpoint now reconstructs user turns from `questions[]`, passes `thread_id` and `feedback` through, and the dialog renders a friendly note pointing to the Foundry thread for the full transcript instead of empty cards. (5) On the Configuration tab the **Reload settings cache** button label was unclear to operators; it is renamed to **Refresh from App Configuration** and both footer buttons now have sentence-case info tooltips that explain what each one actually does (one re-reads values from App Configuration, the other refreshes the running app's settings cache so saved changes take effect without a container restart). Fixes [`Azure/gpt-rag-orchestrator#246`](https://github.com/Azure/gpt-rag-orchestrator/pull/246).

**[Release 2.9.15](https://github.com/Azure/GPT-RAG/tree/v2.9.15) - Schedules tab and auto-dismissing Run now toast**

Patch release that bumps the ingestion pin from `v2.4.12` to [`v2.4.13`](https://github.com/Azure/gpt-rag-ingestion/releases/tag/v2.4.13). No other component changed; orchestrator, UI, and AI Landing Zone pins are identical to v2.9.14. Two operator-reported UX fixes on the ingestion dashboard. The Jobs tab used to stack two operator-control panels above the recent-runs list (the seven-button *Run now* strip and the *Queue and schedule* table), which crowded the page and split operator attention between "trigger and schedule jobs" and "watch what is running right now". Both panels now live on a new dedicated **Schedules** tab inserted between *Jobs* and *Files*, so the tab order is **Jobs | Schedules | Files | Configuration**. The Jobs tab is back to a clean recent-runs view (search, type filter, refresh, runs table); all triggering and scheduling behavior is preserved on the new tab, including the 1-second burst polling after each *Run now* click, the cron column, and the *Last run* summary per job type. Clicking *Run now* on the Schedules tab keeps the operator on Schedules so they can watch the queue update. Separately, the green "Started <job_type>." success toast that appears after *Run now* no longer stays on screen forever — it now auto-dismisses after 4 seconds, each toast has its own independent timer (a fresh trigger does not extend an older toast), and there is a manual close button so operators can dismiss it earlier. The warning toast for "already running" and the error toast follow the same behavior. Fixes [`Azure/gpt-rag-ingestion#254`](https://github.com/Azure/gpt-rag-ingestion/pull/254).
