...
- **Fabric IQ grounding (preview, v3.4.0).** Answers can now include analytical data from a Microsoft Fabric ontology (a semantic model, lakehouse, warehouse, or KQL database exposed through that ontology) through Foundry IQ, blended alongside documents and Work IQ. Off by default. Prereqs at a glance: signed-in user, Fabric capacity and workspace, an ontology item, tenant-enabled Fabric ontology feature, Fabric licenses for target users, same Entra tenant, Foundry IQ preview API `2026-05-01-preview`. Fabric IQ may route request data outside your Foundry region's compliance boundary, so review data residency before turning it on. Live Fabric validation was deferred to a follow-up patch; the shipped code path is covered by unit tests. See [Foundry IQ: Fabric IQ](howto_grounding_fabric_iq.md) and the [Grounding sources overview](howto_grounding_overview.md).
