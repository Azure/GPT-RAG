...
Foundry IQ knowledge base. Existing deployments should stay on
`RETRIEVAL_BACKEND=ai_search` unless you explicitly migrate.

Use the [retrieval backend selection guide](howto_retrieval_backend.md) before
setting `RETRIEVAL_BACKEND=foundry_iq`. It explains Pattern A vs Pattern B,
security modes, `knowledgeRetrieval` billing, rollback, and the post-provision
knowledge base step.
...
| --- | --- | --- |
| `RETRIEVAL_BACKEND` | `ai_search` or `foundry_iq` | Selects the retrieval path. |
| `FOUNDRY_IQ_PATTERN` | `searchIndex` or `managed` | Selects Pattern B existing index or Pattern A managed source. |
...
| `FOUNDRY_IQ_API_VERSION` | `2026-05-01-preview` | Required for per-user permissions and Pattern B `filterAddOn`. |
| `FOUNDRY_IQ_KNOWLEDGE_RETRIEVAL_BILLING_PLAN` | `free` or `standard` | Controls Azure AI Search agentic retrieval billing. |

For Pattern B, run the data-plane knowledge base script after provisioning. In
network-isolated deployments, run it from the jumpbox or another VNet-connected
host.
