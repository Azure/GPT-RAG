...
| Chat/backend configuration contract | `CHAT_BACKEND` values remain `orchestrator` and `hosted_agent`. The default-changing release selects `hosted_agent` only for fresh environments and preserves the persisted selector for upgrades. During an explicit classic-to-hosted migration, the prepare-only provision retains the classic runtime and does not publish `hosted_agent`; cutover occurs only after the immutable hosted image, hosted endpoint, and compatible component deployments are materialized and a successful-cutover marker is recorded. Fresh hosted remains fail-closed and never routes through classic. `DEPLOY_ADMINISTRATIVE_PANEL` remains default `false`. App Configuration label `gpt-rag` must publish backend selector last, after both endpoint outputs, so the UI switches through an explicit deployment operation. | Azure/GPT-RAG, gpt-rag-ui | Missing selector/endpoint outputs, ambiguous key ownership, implicit or premature upgrade migration, or request-time backend fallback. |
...
- An explicit classic-to-hosted migration is two-phase: the prepare-only
  provision retains the classic orchestrator and selector; the selector changes
  only after the digest-pinned hosted deployment returns its endpoint, all
  selected pinned components deploy successfully, and the lifecycle records
  the successful cutover.
- Both pre-provision hooks fetch and detach the AI Landing Zone at
  `manifest.json`'s exact `ailz_commit`; a moving branch head is never used as
