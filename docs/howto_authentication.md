...
To add GPT-RAG chat to an existing portal, follow the
[Chainlit Copilot embedding guide](https://github.com/Azure/gpt-rag-ui/blob/1ef6f9e88022e3168b8caf9c867f090eb632f562/docs/copilot-embedding.md).
It covers the opt-in configuration, deployment topology, anonymous and Entra
authentication modes, browser security controls, and rollout.

!!! warning "Zero Trust deployments"
    The GPT-RAG UI Container App has no public endpoint in a Zero Trust
    deployment. Route widget traffic through the environment's existing
    authenticated front door or gateway. Do not expose the internal Container
...
!!! note "Work IQ and Fabric auth"
    [Work IQ](howto_grounding_work_iq.md),
    [Fabric ontology](howto_grounding_fabric_ontology.md), and
    [Fabric Data Agent](howto_grounding_fabric_data_agent.md) always
    require the same OBO
